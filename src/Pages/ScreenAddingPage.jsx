...
const AddScreenForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      editScreen
        ? await axios
            .put(`http://localhost:7000/screen/update/?_id=${_id}`, formData, {
              headers: { Authorization: `Bearer ${authToken}` },
            })
            .then((res) => {
              toast.success(res.data.Message);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {['screenNo', 'totalSeats', 'firstClassSeats', 'secondClassSeats'].map((field) => (
             ...
             <input
               ...
               className="mt-1 block w-full px-3 py-2 bg-gray-700 text-white border border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500 sm:text-sm no-spinner"
               required
             />
           </div>
