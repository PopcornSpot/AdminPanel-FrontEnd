...
const OverviewPage = () => {
  ...
  const fetchTotalUsers = async () => {
    try {
      await axios
        .get("https://popcornspotbackend-production.up.railway.app/user/getalluser")
        .then((res) => {
  ...
  const fetchTotalTickets = async (theatreId) => {
    try {
      const response = await axios.get(
        `https://popcornspotbackend-production.up.railway.app/payment/getallticketsadmin/?_id=${theatreId}`
      );
  ...
  const fetchTheatre = async () => {
    try {
      const response = await axios.get("https://popcornspotbackend-production.up.railway.app/theatre/get", {
        headers: { Authorization: `Bearer ${authToken}` },
      });
      const fetchedTheatres = response.data.theatres;
      setTheatres(fetchedTheatres);
      return fetchedTheatres;
    } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const fetchedTheatres = await fetchTheatre();

        if (fetchedTheatres.length > 0) {
          await fetchTotalTickets(fetchedTheatres[0]._id);
        } else {
          console.error("Theatres array is empty");
        }
  ...
  useEffect(() => {
    ...
    const previousMonthUserCount =
      groupUsersByMonth[last5Months[1]?.monthKey] || 0;
    const previousMonthRevenue =
      groupTicketsByMonth[last5Months[1]?.monthKey]?.revenue || 0;
    ...
    const updatedOverviewDetails = [
      ...
      },
      {
        ...
        value: (
          <>
            <PiCurrencyInrBold className="inline text-3xl pb-2" /> {totalRevenue}
          </>
        ),
    ...
    if (totalTicket.length > 0) {
      const latestTicket = totalTicket.reduce((prev, current) =>
    ...
    if (totalUser.length > 0) {
      const latestUser = totalUser.reduce((prev, current) =>
        new Date(prev.createdAt) > new Date(current.createdAt) ? prev : current
      );
      recentActivities.push({
    ...
    if (totalMovie.length > 0) {
      const latestMovie = totalMovie.reduce((prev, current) =>
        new Date(prev.createdAt) > new Date(current.createdAt) ? prev : current
      );
      recentActivities.push({
          ...
          <div className="mt-8">
            ...
            <ul className="space-y-4">
              {recentActivity.map((activity, index) => (
                <li
                  key={index}
                  className="bg-gray-800 shadow-md p-4 rounded-lg hover:bg-gray-700 hover:shadow-lg duration-150"
                >
