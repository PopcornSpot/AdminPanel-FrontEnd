    ...
    <div className="min-h-screen bg-gray-900 text-white px-6 py-8 flex flex-col items-center">
        ...
        <table className="w-full bg-gray-800 border border-gray-700 rounded-lg overflow-hidden">
          ...
          <tbody>
            {shows.map((show, index) => (
              <tr
                key={show._id}
                className={`border-b border-gray-700 transition duration-300 hover:bg-gray-600 ${index % 2 === 0 ? "bg-gray-800" : "bg-gray-700"}`}
              >
                <td className="px-6 py-4 font-semibold">{show.movie || "Movie Name"}</td>
                <td className="px-6 py-4">{show.showDate || "Not specified"}</td>
                <td className="px-6 py-4">{show.showTime.length > 0 ? show.showTime.join(", ") : "Not specified"}</td>
      ...
      {selectedShow && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 p-4">
          <div className="bg-gray-800 text-white rounded-lg shadow-lg p-6 w-full max-w-lg animate-fadeIn border border-gray-700">
